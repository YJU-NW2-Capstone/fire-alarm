...
# 로깅 설정 (로그는 mqtt_log.log 파일에 기록됩니다)
logger = logging.getLogger("MQTT_Logger")
logger.setLevel(logging.DEBUG)

# TimedRotatingFileHandler를 사용하여 매일 자정마다 로그 파일 회전, 최근 7일치 로그 백업
...
mqtt_client = mqtt.Client()  # 앱 시작 시 한 번 생성
is_first_connect = True

def load_logs_from_file():
    """mqtt_log.log 파일에 기록된 이전 로그들을 불러옵니다."""
    logs = []
...
def create_log_page(page: ft.Page) -> ft.Container:
    ...
    def add_log(message: str):
        """새로운 로그 메시지를 생성하여 기록하고, 파일과 UI ListView에 기록합니다."""
    ...
    # MQTT 클라이언트는 이미 메인에서 설정되었으므로 여기서는 추가 설정 없이 로그만 기록됩니다.
    return content
...
if __name__ == "__main__":
    def main(page: ft.Page):
        ...
        # 로그 페이지에 기록될 add_log 함수를 미리 정의하고,
        # MQTT 클라이언트의 초기 설정(콜백 등록, 연결)을 앱 시작 시 한 번만 실행합니다.
        def add_log(message: str):
            # 임시 ListView가 없을 때는 단순 출력 (또는 추후 UI 업데이트 시 활용)
            print(message)
